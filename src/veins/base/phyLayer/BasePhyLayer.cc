...
void BasePhyLayer::handleMessage(cMessage* msg)
{
    ...
    if (msg->isSelfMessage()) {
        ...
        numeroBits += check_and_cast<cPacket*>(msg)->getBitLength();//numero de bits de la trama
...
void BasePhyLayer::handleAirFrameStartReceive(AirFrame* frame)
{
    ...
    if (usePropagationDelay) {
        ...
        retardo=delay.dbl()+s.getDuration().dbl(); //Almaceno el retardo de propagacion
...
void BasePhyLayer::handleAirFrameEndReceive(AirFrame* frame)
{
    ...
    tiempoSim = simTime().dbl();
    double velocidad = numeroBits / tiempoSim;
    throughputVec.record(velocidad);

        // Registrar el retardo
    retrasoTotal.record(retardo);
